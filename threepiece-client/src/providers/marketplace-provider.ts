...
const allListedLandsNfts = await getAllListedLandsNfts(
  await activeTradeCounter({
...
export async function getAllListedLandsNfts(numOfLands: bigint) {
  ...
  for (let index = 0; index < numOfLands; index++) {
    const listing = activeTrades({
      // lister address, nft id, price, status
      arg_0: BigInt(index),
      contract: marketplaceLandContract,
    });
  ...
  return (await Promise.all(listedPromise)).filter(
    (l) => l[0] !== "0x0000000000000000000000000000000000000000"
  ); // means inactive trade
}
